  ...
  const OnSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(object.split(' ').length>1){
        return toast('Enter only one object to remove')
      }
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-gray-300">
      ...
      >
        ...
        <textarea
          ...
          value={object}
          rows={4}
          className="w-full p-2 mb-6 px-3 mt-2 outline-none text-sm rounded-md border border-gray-800 text-[#07e8e4] placeholder-gray-500"
          placeholder="Enter object to remove (e.g., person, tree). Only one object at a time"
          required
        />
        ...
        {/* <button
          className="w-full flex justify-center items-center gap-2 relative overflow-hidden 
             ...
             active:scale-95"
        >
          <Scissors className="w-5 relative z-10" />
          <span className="relative z-10">Remove object</span>
        </button> */}
        <button
          disabled={loading}
          className={`w-full flex justify-center items-center gap-2 relative overflow-hidden 
      ...
      <div className="w-full max-w-lg p-4 bg-transparent backdrop-blur-2xl rounded-lg flex flex-col border border-gray-800 min-h-96 ">
        <div className="flex items-center gap-3">
          <Scissors className="w-5 h-5 text-[#07e8e4]" />
